...
@can('member_review_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.member-reviews.create') }}">
                        ...
                        <tr data-entry-id="{{ $memberReview->id }}">
                            ...
                            <td>
                                @can('member_review_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.member-reviews.show', $memberReview->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('member_review_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.member-reviews.edit', $memberReview->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('member_review_delete')
                                    <form action="{{ route('admin.member-reviews.destroy', $memberReview->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('member_review_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.member-reviews.massDestroy') }}",
