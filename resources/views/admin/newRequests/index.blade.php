...
@can('new_request_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.new-requests.create') }}">
                        ...
                        <tr data-entry-id="{{ $newRequest->id }}">
                            ...
                            <td>
                                @can('new_request_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.new-requests.show', $newRequest->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('new_request_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.new-requests.edit', $newRequest->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('new_request_delete')
                                    <form action="{{ route('admin.new-requests.destroy', $newRequest->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('new_request_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.new-requests.massDestroy') }}",
